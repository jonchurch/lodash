...
function postprocess(markdown) {
  ...
  markdown = markdown.replace(/\.(Symbol\.(?:[a-z]+[A-Z]?)+)/g, '[$1]');
  // Convert <h3> tags to markdown headings so GitHub renders links correctly.
  markdown = markdown.replace(/<h3[^>]*>(?:<a[^>]*>[^<]*<\/a>)?<code>([^<]+)<\/code><\/h3>/g, '### `$1`');
  return markdown;
}
