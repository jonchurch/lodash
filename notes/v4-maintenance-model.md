...
Each dist branch owns its own build workflow. The build script lives on the branch, accepts a ref (tag, SHA, branch name) or local directory path, and knows how to produce that branch's distribution files. The CLI handles ref resolution itself via `--ref` — it fetches the full source tree from GitHub as a tarball, so no local git repo is needed. For local dev, `--source-dir` points at a directory. The build script is pure orchestration (which CLI commands to run for this branch), not plumbing. The branch is self-describing — a maintainer can look at it and understand how it's built without knowing anything about main's workflows.
