...
(function () {
  ...
  QUnit.test('should accept a falsey `thisArg`', function (assert) {
    ...
    assert.ok(
      lodashStable.every(actual, function (value, index) {
  ...
  QUnit.test(
    'should handle a number of arguments when called with the `new` operator',
    function (assert) {
      assert.expect(1);
  ...
  QUnit.test(
    'should not error when calling bound class constructors with the `new` operator',
    function (assert) {
      assert.expect(1);
