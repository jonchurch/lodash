...
(function () {
  ...
  QUnit.test('should coerce nullish `omission` values to strings', function (assert) {
    ...
    assert.strictEqual(
      _.truncate(string, { omission: undefined }),
  ...
  QUnit.test('should support a `separator` option', function (assert) {
    ...
    assert.strictEqual(
      _.truncate(string, { length: 24, separator: ' ' }),
      'hi-diddly-ho there,...'
    );
    assert.strictEqual(
      _.truncate(string, { length: 24, separator: /,? +/ }),
    ...
    assert.strictEqual(
      _.truncate(string, { length: 24, separator: /,? +/g }),
      'hi-diddly-ho there...'
    );
  });
  ...
  QUnit.test('should coerce `length` to an integer', function (assert) {
    ...
    lodashStable.each(['', NaN, 4.6, '4'], function (length, index) {
      ...
      assert.strictEqual(
        _.truncate(string, { length: { valueOf: lodashStable.constant(length) } }),
        actual
      );
    });
  });
  ...
  QUnit.test('should coerce `string` to a string', function (assert) {
    ...
    assert.strictEqual(
      _.truncate({ toString: lodashStable.constant(string) }, { length: 5 }),
      'hi...'
    );
  });
