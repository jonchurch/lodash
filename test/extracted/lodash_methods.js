...
(function () {
  ...
  QUnit.test('should throw an error for falsey arguments', function (assert) {
    ...
    lodashStable.each(rejectFalsey, function (methodName) {
      ...
      var actual = lodashStable.map(falsey, function (value, index) {
        var pass =
          !index && /^(?:backflow|compose|cond|flow(Right)?|over(?:Every|Some)?)$/.test(methodName);
  ...
  QUnit.test(
    'should not contain minified method names (test production builds)',
    function (assert) {
      assert.expect(1);

      var shortNames = ['_', 'at', 'eq', 'gt', 'lt'];
      assert.ok(
        lodashStable.every(_.functions(_), function (methodName) {
