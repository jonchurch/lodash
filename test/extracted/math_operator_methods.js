...
var isNpm =
  typeof require === 'function' && !require.resolve.toString().match(/\[native code\]/)
    ? true
    : false;
...
lodashStable.each(['add', 'divide', 'multiply', 'subtract'], function (methodName) {
  ...
  QUnit.test(
    '`_.' + methodName + '` should return `' + (isAddSub ? 0 : 1) + '` when no arguments are given',
    function (assert) {
      assert.expect(1);
  ...
  QUnit.test(
    '`_.' + methodName + '` should work with only one defined argument',
    function (assert) {
      assert.expect(3);
