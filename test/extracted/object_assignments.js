...
lodashStable.each(['assignWith', 'assignInWith', 'mergeWith'], function (methodName) {
  ...
  QUnit.test(
    ...
    function (assert) {
      ...
      func(object, source, function () {
        args ||
          (args = lodashStable.map(
            Array.prototype.slice.call(arguments, 0, 5),
      ...
      expected = [
        lodashStable.map([objectValue, sourceValue, 'a', object, source], lodashStable.cloneDeep),
      ];

      if (isMergeWith) {
        expected.push(
          lodashStable.map([undefined, 2, 'b', objectValue, sourceValue], lodashStable.cloneDeep)
        );
      }
      func(object, source, function () {
        argsList.push(
          lodashStable.map(Array.prototype.slice.call(arguments, 0, 5), lodashStable.cloneDeep)
        );
      });
  ...
  QUnit.test(
    '`_.' + methodName + '` should not treat the second argument as a `customizer` callback',
    function (assert) {
      assert.expect(2);
