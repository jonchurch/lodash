...
lodashStable.each(['range', 'rangeRight'], function (methodName) {
  ...
  QUnit.test(
    '`_.' + methodName + '` should infer the sign of `step` when only `end` is given',
    function (assert) {
      assert.expect(2);
  ...
  QUnit.test(
    '`_.' + methodName + '` should infer the sign of `step` when only `start` and `end` are given',
    function (assert) {
      assert.expect(2);
  ...
  QUnit.test('`_.' + methodName + '` should coerce arguments to finite numbers', function (assert) {
    ...
    var actual = [func('1'), func('0', 1), func(0, 1, '1'), func(NaN), func(NaN, NaN)];
