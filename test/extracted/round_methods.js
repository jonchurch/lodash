...
lodashStable.each(['ceil', 'floor', 'round'], function (methodName) {
  ...
  QUnit.test(
    '`_.' + methodName + '` should work with exponential notation and `precision`',
    function (assert) {
      assert.expect(3);
  ...
  QUnit.test(
    ...
    function (assert) {
      ...
      var results = [_.round(10.0000001, 1000), _.round(MAX_SAFE_INTEGER, 293)];
  ...
  QUnit.test(
    '`_.' + methodName + '` should return `Infinity` given `Infinity` regardless of `precision`',
    function (assert) {
      assert.expect(6);
  ...
  QUnit.test(
    '`_.' + methodName + '` should return `NaN` given `NaN` regardless of `precision`',
    function (assert) {
      assert.expect(6);
