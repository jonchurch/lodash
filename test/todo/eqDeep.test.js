...
describe('eqDeep', () => {
  ...
  it('should compare primitives', () => {
    ...
    const actual = lodashStable.map(pairs, (pair) => eqDeep(pair[0], pair[1]))
  ...
  it('should compare arrays', () => {
    ...
    assert.strictEqual(eqDeep(array1, array2), true)
    ...
    assert.strictEqual(eqDeep(array1, array2), false)
  ...
  it('should compare sparse arrays', () => {
    ...
    assert.strictEqual(eqDeep(array, Array(1)), true)
    assert.strictEqual(eqDeep(array, [undefined]), true)
    assert.strictEqual(eqDeep(array, Array(2)), false)
  })
  ...
  it('should compare plain objects', () => {
    ...
    assert.strictEqual(eqDeep(object1, object2), true)
    ...
    assert.strictEqual(eqDeep(object1, object2), false)
  ...
  it('should compare object instances', () => {
    ...
    assert.strictEqual(eqDeep(new Foo, new Foo), true)
    assert.strictEqual(eqDeep(new Foo, new Bar), false)
    assert.strictEqual(eqDeep({ 'a': 1 }, new Foo), false)
    assert.strictEqual(eqDeep({ 'a': 2 }, new Bar), false)
  })

  it('should compare objects with constructor properties', () => {
    assert.strictEqual(eqDeep({ 'constructor': 1 },   { 'constructor': 1 }), true)
    assert.strictEqual(eqDeep({ 'constructor': 1 },   { 'constructor': '1' }), false)
    assert.strictEqual(eqDeep({ 'constructor': [1] }, { 'constructor': [1] }), true)
    assert.strictEqual(eqDeep({ 'constructor': [1] }, { 'constructor': ['1'] }), false)
    assert.strictEqual(eqDeep({ 'constructor': Object }, {}), false)
  })
  ...
  it('should have transitive equivalence for circular references of arrays', () => {
    ...
    assert.strictEqual(eqDeep(array1, array2), true)
    assert.strictEqual(eqDeep(array2, array3), true)
    assert.strictEqual(eqDeep(array1, array3), true)
  })
  ...
  it('should have transitive equivalence for circular references of objects', () => {
    ...
    assert.strictEqual(eqDeep(object1, object2), true)
    assert.strictEqual(eqDeep(object2, object3), true)
    assert.strictEqual(eqDeep(object1, object3), true)
  })
  ...
  it('should treat objects created by `Object.create(null)` like plain objects', () => {
    ...
    assert.strictEqual(eqDeep(object1, object2), true)
    assert.strictEqual(eqDeep(new Foo, object2), false)
  })

  it('should avoid common type coercions', () => {
    assert.strictEqual(eqDeep(true, Object(false)), false)
    assert.strictEqual(eqDeep(Object(false), Object(0)), false)
    assert.strictEqual(eqDeep(false, Object('')), false)
    assert.strictEqual(eqDeep(Object(36), Object('36')), false)
    assert.strictEqual(eqDeep(0, ''), false)
    assert.strictEqual(eqDeep(1, true), false)
    assert.strictEqual(eqDeep(1337756400000, new Date(2012, 4, 23)), false)
    assert.strictEqual(eqDeep('36', 36), false)
    assert.strictEqual(eqDeep(36, '36'), false)
  })
  ...
  it('should compare `arguments` objects', () => {
    ...
    assert.strictEqual(eqDeep(args1, args2), true)
    assert.strictEqual(eqDeep(args1, args3), false)
  })
  ...
  it('should treat `arguments` objects like `Object` objects', () => {
    ...
    assert.strictEqual(eqDeep(args, object), true)
    assert.strictEqual(eqDeep(object, args), true)
    assert.strictEqual(eqDeep(args, new Foo), false)
    assert.strictEqual(eqDeep(new Foo, args), false)
  })
  ...
  it('should compare array buffers', () => {
    if (ArrayBuffer) {
      ...
      assert.strictEqual(eqDeep(buffer, new Uint8Array([255]).buffer), true)
      assert.strictEqual(eqDeep(buffer, new ArrayBuffer(1)), false)
    }
  })
  ...
  it('should compare buffers', () => {
    if (Buffer) {
      ...
      assert.strictEqual(eqDeep(buffer, new Buffer([1])), true)
      assert.strictEqual(eqDeep(buffer, new Buffer([2])), false)
      assert.strictEqual(eqDeep(buffer, new Uint8Array([1])), false)
    }
  })
  ...
  it('should compare date objects', () => {
    ...
    assert.strictEqual(eqDeep(date, new Date(2012, 4, 23)), true)
    assert.strictEqual(eqDeep(new Date('a'), new Date('b')), true)
    assert.strictEqual(eqDeep(date, new Date(2013, 3, 25)), false)
    assert.strictEqual(eqDeep(date, { 'getTime': lodashStable.constant(+date) }), false)
  })
  ...
  it('should compare functions', () => {
    ...
    assert.strictEqual(eqDeep(a, a), true)
    assert.strictEqual(eqDeep(a, b), false)
  })
  ...
  it('should compare maps', () => {
    if (Map) {
      lodashStable.each([[map, new Map], [map, realm.map]], (maps) => {
        ...
        map1.set('a', 1)
        map2.set('b', 2)
        assert.strictEqual(eqDeep(map1, map2), false)

        map1.set('b', 2)
        map2.set('a', 1)
        assert.strictEqual(eqDeep(map1, map2), true)
  ...
  it('should compare promises by reference', () => {
    if (promise) {
      lodashStable.each([[promise, Promise.resolve(1)], [promise, realm.promise]], (promises) => {
        ...
        assert.strictEqual(eqDeep(promise1, promise2), false)
        assert.strictEqual(eqDeep(promise1, promise1), true)
      })
    }
  })

  it('should compare regexes', () => {
    assert.strictEqual(eqDeep(/x/gim, /x/gim), true)
    assert.strictEqual(eqDeep(/x/gim, /x/mgi), true)
    assert.strictEqual(eqDeep(/x/gi, /x/g), false)
    assert.strictEqual(eqDeep(/x/, /y/), false)
    assert.strictEqual(eqDeep(/x/g, { 'global': true, 'ignoreCase': false, 'multiline': false, 'source': 'x' }), false)
  })
  ...
  it('should compare sets', () => {
    if (Set) {
      lodashStable.each([[set, new Set], [set, realm.set]], (sets) => {
        ...
        set1.add(1)
        set2.add(2)
        assert.strictEqual(eqDeep(set1, set2), false)

        set1.add(2)
        set2.add(1)
        assert.strictEqual(eqDeep(set1, set2), true)
  ...
  it('should compare wrapped values', () => {
    ...
    lodashStable.each(values, (vals) => {
      ...
      assert.strictEqual(eqDeep(_(actual), _(true)), true)
      ...
      assert.strictEqual(eqDeep(_(actual), _(false)), true)
    })
  })
  ...
  it('should work as an iteratee for `_.every`', () => {
    const actual = lodashStable.every([1, 1, 1], lodashStable.partial(eqDeep, 1))
    assert.ok(actual)
  })
  ...
  it('should not error on DOM elements', () => {
    if (document) {
      ...
      try {
        assert.strictEqual(eqDeep(element1, element2), false)
      } catch (e) {
  ...
  it('should return `true` for like-objects from different documents', () => {
    if (realm.object) {
      assert.strictEqual(eqDeep([1], realm.array), true)
      assert.strictEqual(eqDeep([2], realm.array), false)
      assert.strictEqual(eqDeep({ 'a': 1 }, realm.object), true)
      assert.strictEqual(eqDeep({ 'a': 2 }, realm.object), false)
    }
  })
  ...
  it('should return a wrapped value when explicitly chaining', () => {
    // This is really testing chain moreso than eqDeep?
    // likely this test should be deleted
